...
class MgBeloHorizonteSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.FormRequest(
            ...
            headers=headers,
            callback=self.parse,
            meta={'page_number': 1, 'formdata': formdata, 'headers': headers}
        )
    ...
    def parse(self, response):
        ...
        if resumo:
            match_total = re.search(r'de um total de (\d+) itens', resumo)
            match_range = re.search(r'itens de (\d+) a (\d+)', resumo)
